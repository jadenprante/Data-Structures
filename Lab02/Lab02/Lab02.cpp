...
*
* Overview:
* The DoublyLinkedList lab will allow traversal  
* in both forward and reverse directions due
* to the presence of two pointers (previous and next) in each node
* 
* Input:
* 15 strrings which are names of students, these will used
* to complete a serioes of test below.
*
* Output:
* Once the program runs, it should run the test code(s) 
* below and will output a pass or fail, depending 
* on the result it obtainss.
*
************************************************************/
...
DoublyLinkedList<int> ReturnIntList();
DoublyLinkedList<string> ReturnStrList();
...
test_append, test_purge, test_extract_empty, test_extract_match,
...
test_is_empty_complex, test_first_with_value_complex,
test_first_empty_list_complex,
test_last_with_value_complex, test_last_empty_list_complex,
test_prepend_complex, test_append_complex, test_purge_complex,
test_extract_empty_complex, test_extract_match_complex,
test_extract_no_match_complex,
test_inserta_empty_complex, test_inserta_match_complex,
test_inserta_no_match_complex,
test_insertb_empty_complex, test_insertb_match_complex,
test_insertb_no_match_complex };
int main()
{
...
bool test_default_ctor()
{
	...
	DoublyLinkedList<int> list_test;
...
bool test_copy_ctor()
{
	...
	for (int i = 0; i < NUM_SIZE; ++i)
		test_list.Append(i);
	DoublyLinkedList<int> list_test(test_list);
...
{
	bool pass = true;
	DoublyLinkedList<int> test_list(ReturnIntList());
	// Check data integrity
	for (int i = 0; i < NUM_SIZE; ++i)
	{
...
{
	bool pass = true;
	DoublyLinkedList<int> full_list;
	for (int i = 0; i < NUM_SIZE; ++i)
		full_list.Append(i);
...
bool test_default_ctor_complex()
{
	...
	DoublyLinkedList<string> list_test;
	if (list_test.getHead() != default_head)
		pass = false;
	if (list_test.getTail() != default_tail)
		pass = false;
...
bool test_copy_ctor_complex()
{
	...
	DoublyLinkedList<string> list_test(test_list);
	// Check data integrity
	for (int i = 0; i < NUM_NAMES; ++i)
	{
		if (list_test.First() != test_list.First())
...
{
	bool pass = true;
	DoublyLinkedList<string> test_list(ReturnStrList());
	// Check data integrity
	for (int i = 0; i < NUM_NAMES; ++i)
	{
...
{
	bool pass = true;
	DoublyLinkedList<string> full_list;
	for (int i = 0; i < NUM_NAMES; ++i)
		full_list.Append(NAMES[i]);
	DoublyLinkedList<string> empty_list;
	if (full_list.isEmpty())
		pass = false;
	if (!empty_list.isEmpty())
		pass = false;
...
DoublyLinkedList<string> ReturnStrList()
{
	DoublyLinkedList<string> tempList;
	for (int i = 0; i < NUM_NAMES; ++i)
